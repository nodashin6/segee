...
sum_spec = importlib.util.spec_from_file_location("sum_solve", Path(__file__).parent / "01_range_sum_query" / "sum_solve.py")
sum_module = importlib.util.module_from_spec(sum_spec)
sum_spec.loader.exec_module(sum_module)

sum_naive_spec = importlib.util.spec_from_file_location("sum_groundtruth", Path(__file__).parent / "01_range_sum_query" / "sum_groundtruth.py")
...
min_spec = importlib.util.spec_from_file_location("min_solve", Path(__file__).parent / "02_range_min_query" / "min_solve.py")
min_module = importlib.util.module_from_spec(min_spec)
min_spec.loader.exec_module(min_module)

min_naive_spec = importlib.util.spec_from_file_location("min_groundtruth", Path(__file__).parent / "02_range_min_query" / "min_groundtruth.py")
...
class TestPerformanceComparison:
    ...
    def test_sum_query_performance(self, n: int, q: int) -> None:
        ...
        # For small sizes, segment tree might have initialization overhead
        # But for larger sizes, it should show advantage
        if n >= 1000 and q >= 1000:
            # Segment tree should be faster than naive, even if just slightly
            assert segment_time < naive_time, \
                f"Segment tree should be faster: {segment_time:.4f}s vs {naive_time:.4f}s"
        else:
            # For small inputs, just verify both complete successfully
    ...
    def test_scalability_demonstration(self) -> None:
        ...
        for n, q in sizes:
            ...
            # For larger inputs, segment tree should definitely show advantage
            # Even a slight advantage is sufficient
            assert segment_time < naive_time, \
                f"Segment tree should be faster for large inputs: {segment_time:.4f}s vs {naive_time:.4f}s"
